/**
 * @file app/components/features/ProspectCardWithActions.tsx
 * @description Enhanced prospect card with suggested actions
 */

'use client';

import { useState } from 'react';
import type { Prospect, SuggestedAction } from '@/types';
...
import { useSuggestedActions } from '@/app/hooks/useSuggestedActions';

interface ProspectCardWithActionsProps {
  prospect: Prospect;
  onCall?: () => void;
  onEmail?: () => void;
  onClick?: () => void;
...
export default function ProspectCardWithActions({
  prospect,
  onCall,
  onEmail,
  onClick,
  onActionClick,
  showActions = true,
}: ProspectCardWithActionsProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const { actions, isLoading: actionsLoading } = useSuggestedActions(
    showActions && isExpanded ? prospect.id : null
  );
  ...
  return (
    ...
    >
      ...
      {showActions && isExpanded && (
        <div className="mt-4">
          {actionsLoading ? (
            <div className="text-sm text-[#8E99A4]">Loading actions...</div>
          ) : (
            <SuggestedActions
              actions={actions}
