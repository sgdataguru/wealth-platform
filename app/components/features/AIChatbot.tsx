...
1. **Ahmad Al Maktoum** (Score: 92)
   IPO filing detected - High priority
   
2. **Layla Al-Mazrouei** (Score: 87)
   Recent funding round announced

Would you like me to provide more details on any of these prospects?`,
        timestamp: new Date(),
        links: [
          { label: 'View Ahmad Al Maktoum', href: '/prospects/1' },
          { label: 'View Layla Al-Mazrouei', href: '/prospects/2' },
        ],
      };
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-[#E85D54] to-[#F06E66] rounded-full shadow-lg flex items-center justify-center text-[#1A1A2E] hover:shadow-xl transition-all duration-300 hover:scale-105 z-50"
      >
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-[#1A1332] rounded-t-xl">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-r from-[#E85D54] to-[#F06E66] rounded-full flex items-center justify-center">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {message.links.map((link, index) => (
                        <a
                          key={index}
                          href={link.href}
                          className="text-xs text-[#E85D54] hover:underline"
                        >
                          {link.label}
                        </a>
                      ))}
            ...
            <div className="flex flex-wrap gap-2">
              {quickQuestions.map((question) => (
                <button
                  ...
                  className="px-3 py-1.5 text-xs bg-[#F8F9FA] text-[#5A6C7D] rounded-full hover:bg-[#2A2447] hover:text-white transition-colors"
                >
                  {question}
                </button>
              ))}
          ...
          <div className="p-4 border-t border-gray-100">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 bg-[#F8F9FA] border border-gray-200 rounded-lg text-[#1A1A2E] placeholder-[#8E99A4] focus:outline-none focus:border-[#2A2447] transition-colors"
              />
